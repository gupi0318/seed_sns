<?php 
    ...
    // $spl= "SELECT * FROM `tweets` LEFT JOIN `members` ON `members`.`member_id`=`tweets`.`member_id` WHERE `nick_name`=?,`picture_path`=?,`delete_flag=0`";
    // $data=($_GET['tweet_id']['nick_name'])
    ...
    if (!empty($_POST)) {
      ...
      if(!isset($error)){
        $insert_sql = 'INSERT INTO `tweets` SET `tweet`=?,`member_id`=?,`reply_tweet_id`=? ';
  ...
  <div class="container">
    <div class="row">
      <div class="col-md-6 col-md-offset-3 content-margin-top">
        ...
        <p class = "error"><?php if((isset($error)) && $error['tweet']=='empty'){
          echo 'コメント書きましょう';
        } ?></p>
